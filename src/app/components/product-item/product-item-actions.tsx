...
export const ProductItemAction = ({ product }: ProductItemActionProps) => {
  const { increaseItem, decreaseItem, getItemAmount, setAmount, items, addProduct } = useCarShop()
  ...
  return (
    <div className="flex align-baseline justify-center mt-8 gap-5">
      <button className="rounded-full p-2 bg-red-500" onClick={() => decreaseItem(product.id)}>
        <MdRemove size={24} />
      </button>
      <input
        className="w-30 border rounded-md pl-4"
        value={amountItem}
        type="number"
